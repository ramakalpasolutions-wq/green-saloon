...
export default function CheckInModal({ salon, isOpen, onClose }) {
  ...
  const handleConfirm = () => {
    ...
    setTimeout(() => {
      ...
      // Parse wait time from salon data
      ...
      // ✅ USE THE CAPTURED TIMESTAMP - NOT new Date()
      const bookingDate = bookingTimestamp || new Date();
      
      // Format for display - using the SAME captured time
      ...
      console.log('⏰ CAPTURED BOOKING TIME (ISO):', bookingDate.toISOString());
      ...
      const bookingData = {
        ...
        bookingTimestamp: bookingDate.toISOString(), // ✅ ISO timestamp for calculations
        status: 'confirmed',
      ...
      // ✅ Pass timestamp to booking status page
      const queryParams = new URLSearchParams({
        id: bookingId,
        salon: salon?.name || 'Green Saloon',
        queue: queueNumber.toString(),
        waitTime: actualWaitTime.toString(),
        bookingTime: bookingDate.toISOString() // ✅ Pass ISO timestamp
      });
      
      console.log('🔗 Navigating to:', `/booking-status?${queryParams.toString()}`);
